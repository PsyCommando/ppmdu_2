    ...
    public:
        ...
        */
            ...
            void LoadDataFromBin( _DataTy & out_data, const binarylocatioinfo & bininfo, std::ifstream & binstrm )
        {
            ...
            assert(bininfo.location.end < std::numeric_limits<std::streamoff>::max());

            while(static_cast<std::streamoff>(binstrm.tellg()) < static_cast<std::streamoff>(bininfo.location.end))
            {
                _TransType entry;
        ...
        */
            ...
            _outfwdit DumpStringAndList( _infwdit itbeg, _infwdit itend, _outfwdit itw, bool bputsubheader = true )const
        {
            ...
            for( auto itstr = itbeg; itstr != itend; ++itstr)
            {
                ...
                stroffsets.push_back(static_cast<uint32_t>(outdata.size()));
            ...
            // -----------------------------------------
            uint32_t ptrdatatbl = static_cast<uint32_t>(outdata.size());
            ...
            if(bputsubheader)
            {
                sir0anddat.SetDataPointerOffset(static_cast<uint32_t>(outdata.size()));
                //Append a subheader
