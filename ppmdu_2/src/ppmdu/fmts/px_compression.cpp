...
//=========================================
    ...
    *********************************************************************************/
        ...
        typename px_compressor<_inRandit,_outRandit>::compOp px_compressor<_inRandit,_outRandit>::DetermineBestOperation( ePXCompLevel compressionlvl, bool shouldsearchfirst, uint64_t * pTotalBytesHandled )
    {
        ...
        uint32_t debugoffsettracker = static_cast<uint32_t>(distance(m_itInBeg, m_itInCur));
    ...
    *********************************************************************************/
        ...
        bool px_compressor<_inRandit,_outRandit>::CanCompressTo2In1ByteWithManipulation( inIterRand_t itcurbyte, compOp  & out_result )
    {
        ...
        {
            nymatches[i] = static_cast<uint8_t>(count( nybbles.begin(), nybbles.end(), nybbles[i] ));
        }
        ...
        if( count( nymatches.begin(), nymatches.end(), 3 ) == 3 )
        {
            ...
            if( *(minmaxvalues.second) - *(minmaxvalues.first) == 1 )
            {
                ...
                uint32_t indexsmallest = static_cast<uint32_t>(distance( nybbles.begin(), minmaxvalues.first ));
                uint32_t indexlargest  = static_cast<uint32_t>(distance( nybbles.begin(), minmaxvalues.second ));
    ...
    *********************************************************************************/
        ...
        bool px_compressor<_inRandit,_outRandit>::CanUseAMatchingSequence( inIterRand_t itcurbyte, compOp  & out_result )
    {
        ...
        auto _currentOffset = distance(m_itInBeg, itcurbyte);
        assert((_currentOffset < std::numeric_limits<int32_t>::max()) && (_currentOffset > std::numeric_limits<int32_t>::min()));
        int32_t  currentOffset       = static_cast<int32_t>(_currentOffset);
        ...
        auto _curSeqLen = distance(itSequenceBeg, itSequenceEnd);
        assert(_curSeqLen < std::numeric_limits<uint32_t>::max());
        uint32_t curSeqLen = static_cast<uint32_t>(_curSeqLen);
        ...
        if( result.length >= PX_MIN_MATCH_SEQLEN )
        {
            ...
            if( !CheckSequenceHighNybbleValidOrAdd(static_cast<uint8_t>(result.length - PX_MIN_MATCH_SEQLEN)) )
            {
            ...
            const auto distance_result = distance(result.itpos, itcurbyte);
            assert((distance_result < std::numeric_limits<int16_t>::max()) && (distance_result > std::numeric_limits<int16_t>::min())); //Probably uneeded, but this will warn on overflow
            const int16_t signedoffset = -static_cast<int16_t>(distance_result);
    ...
    *********************************************************************************/
    ...
    {
        uint32_t initialNbOps = static_cast<uint32_t>(m_PendingOperations.size());
        ...
        while( !m_PendingOperations.empty() )
        {
            ...
            *m_itOutCur =  commandbyte;
            ...
            if( pPercentDone != nullptr )
                (*pPercentDone) = static_cast<uint8_t>( ((initialNbOps - static_cast<uint32_t>(m_PendingOperations.size())) * 100u) / initialNbOps );
        }
